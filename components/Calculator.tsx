...
const Calculator: React.FC = () => {
  ...
  const calculateFinalPrice = (distance: number) => {
    ...
    // Este valor progride linearmente com a distância.
    ...
    // Reduzido para ser acessível localmente mas cobrir custos de saída.
    let finalPrice = LOGISTICS_CONFIG.MINIMUM_BASE_PRICE + kmCalculation;

    // 3. Regras de Longa Distância (Acima de 100km)
    // Aplica taxas extras fixas e por km excedente para manter a lucratividade em rotas longas.
  ...
  const generateWhatsAppLink = () => {
    if (!result) return '#';
    const message = `Olá equipe Fretes Mudança Fragata! Gostaria de agendar este serviço:
📍 Origem: ${formData.origin}
🏁 Destino: ${formData.destination}
🛣️ Distância: ${formData.distance}km
💰 Valor Cotado: ${LOGISTICS_CONFIG.CURRENCY} ${result.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}

Como podemos proceder para realizar a mudança?`;
      ...
      <div className="bg-slate-900/40 backdrop-blur-2xl border border-white/5 rounded-[3rem] overflow-hidden shadow-3xl">
        <div className="grid lg:grid-cols-5">
          ...
          <div className="lg:col-span-3 p-8 md:p-12 lg:p-16 border-b lg:border-b-0 lg:border-r border-white/5">
            <div className="mb-10">
              <h3 className="text-3xl font-black text-white font-outfit mb-2 italic uppercase">Calculadora Fragata</h3>
              <p className="text-slate-500 text-sm">Simulação otimizada: Preços justos para trajetos locais e máxima eficiência em longas distâncias.</p>
            </div>
            ...
            <form onSubmit={handleCalculate} className="space-y-8">
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Distância em KM</label>
                <div className="relative">
          ...
          <div className="lg:col-span-2 bg-white/[0.02] p-8 md:p-12 flex flex-col justify-center items-center relative overflow-hidden">
            {!result && !calculating && (
              <div className="text-center space-y-6">
                ...
                <div>
                  <h4 className="text-white font-bold text-lg">Pronto para a Mudança?</h4>
                  <p className="text-slate-500 text-sm max-w-[200px] mx-auto leading-relaxed">Valores equilibrados para todos os tipos de trajetos.</p>
                </div>
              </div>
            )}
            ...
            {calculating && (
              <div className="text-center space-y-8">
                ...
                <p className="text-cyan-400 font-black text-xs uppercase tracking-widest animate-pulse italic">Processando Cálculo...</p>
              </div>
            )}
              ...
              <div className="w-full text-center space-y-10 animate-in fade-in zoom-in duration-500">
                <div className="space-y-4">
                  ...
                  <div className="flex flex-col">
                    <span className="text-slate-500 font-bold mb-1 uppercase text-xs tracking-widest">Valor do Frete</span>
                ...
                <div className="space-y-4 pt-8 border-t border-white/5 w-full max-w-xs mx-auto">
                   <div className="flex justify-between text-[10px] font-black uppercase tracking-wider">
                     ...
                     <span className="text-cyan-400">Mudança Premium</span>
                   </div>
                   <div className="flex justify-between text-[10px] font-black uppercase tracking-wider">
                     <span className="text-slate-500">Garantia</span>
                     <span className="text-white">Fragata 100%</span>
                   </div>
                </div>
